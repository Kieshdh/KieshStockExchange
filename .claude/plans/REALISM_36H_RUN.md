...
**BUILD ORDER (autonomous):** ✅candle step3(serve-stale) / step2 deferred → ✅F2 PARKED (fork→Kiesh) → ✅F1+F5 combined confirm PASSED (deploy HELD→Kiesh eyeball) → ✅candle seam step7 (`6b873f7`, continuous-open behind default-off `Candles:ContinuousOpen`, 761 tests; HOLD-for-eyeball flip like F3) → ✅candle cache steps 4-5 DONE (`b269d19` CandleCache pure class in Shared, 13 tests/774 total; `f1b7b68` WIRED into SignalRCandleService — cache-first GetHistoricalCandlesAsync bucket-aligned + live-merge OnHubCandleClosed, behind default-off `Candles:ClientCache`, client builds clean, byte-identical off; VERIFY on a running client before flipping on) → step6 reconnect tail-refetch = optional robustness (cache self-heals via frontier-miss→refetch without it) → **NEXT = F4 MVVM restructure** (build+761 tests+launch gate, no soak) → **F3 LAST** (charts-in-chat, hold). Await Kiesh: F2 weak-vs-visible fork; F1+F5 prod deploy+ON-flip (F5 breathe eyeball); candle ContinuousOpen flip (chart eyeball). Attribution rule: never co-enable F1/F2/F5 during a solo soak; F1×F2 compound-cap `VolumeMult×H≤SIZE cap`; F1+F5 combined ret_acf must stay OFF±0.05.
...
- **★★★ DEPLOYED + CONSOLIDATED (2026-07-24 ~13:48) — the WHOLE realism arc + drift lever is now LIVE on prod:** merged everything to `origin/master` = `d578c69` (realism arc RegimeTaker/F1/F5/μ-engine/candle + Q7 CK-fix `d784b61`, + chart-tooling arc `5f345b6`, + arb arc — all clean, 778 tests). Prod rebuilt from `d784b61` (server part), **CONFIGCHECK RegimeTaker coupling=True strength=0.12 threshold=0.20 cohortFraction=0.03 bias=0.015 biasSpread=0.0** (= council config B: μ-engine-B drift lever 0.015, spread 0, F5 MarketPulse off). restarts=0, CK=0 since boot, healthy. F1/F5/candle CODE now on prod but DEFAULT-OFF (env not set). ✅ prod admin pw rotated → `Petertje60!`. **★ NOW WATCHING: RegimeTaker + drift bias are NEWLY ACTIVE on prod (first time) — market will SHIFT over hours. Confirm it lands on-target: drift gently POSITIVE-but-close-to-zero, ret_acf stays in-band (RegimeTaker adds momentum — watch it doesn't cross toward 0/positive), moves calm (no >3%), CK=0. Soak OFF-arm proved this config on-target. If off: TakerBias/RegimeTaker are ENV-reversible in seconds.** Early post-deploy read @13:48: moves calm, CK=0. **★ @14:04 Kiesh GO → F1 + F5 FLIPPED ON in prod** (env: `Bots__Personality__SectorSizeModel=true` + `Bots__Sentiment__RegimeDrift__MarketPulse__Enabled=true`; env-only recreate, no rebuild). Verified: CONFIGCHECK `MarketPulse enabled=True`, restarts=0, CK=0. So prod NOW = RegimeTaker + TakerBias 0.015 + F5 breathe + F1 sector×size personalities (3 new dynamics on a settling market). Kiesh eyeballing F5 breathe on prod client. ★ INTENSIFIED WATCH: if ret_acf creeps toward 0 (2 cyc) ⇒ dial TakerBias/RegimeTaker; if moves spiky/>3% ⇒ trim F5 MarketPulse:OscA or RegimeTaker TakerStrength; CK=0 hard. All env-reversible. REMAINING for Kiesh: ContinuousOpen/ClientCache flips; F2 arrival-rate redesign; F4 launch-verify.
  - **★★ μ-engine-B SOAKED + COUNCILLED → SHIP config B (`TakerBias 0.015`, `TakerBiasSpread 0`).** Soak (60m same-seed): CK=0; drift up-shift +0.29%/hr delta; %net-up 29→37%; maxcap 1.02 (bounded, no runaway); ret_acf −0.123→−0.129 (in-band); per-stock drift_std UNCHANGED (spread swamped by regime walk ⇒ drop spread, keep code for a future multi-day soak). Council (5): ship mean-only tuned DOWN; spread is theatre short-term; ★ this lever = a BOUNDED one-time level-lean NOT a sustained weekly climb (that needs a fundamental/anchor nominal-growth drift = separate follow-on).
  - **★★★ DEPLOY BLOCKER (Kiesh chose = merge whole branch):** prod = `6056ce1` runs WITHOUT RegimeTaker/F1/F5/μ-engine/candle — the ENTIRE realism arc is built+soaked on this branch (64 commits ahead of origin/master) but NEVER deployed; prod's RegimeDrift env vars are INERT (code missing). TakerBias REQUIRES RegimeTaker ⇒ shipping config B = deploying the whole arc (activates RegimeTaker+Mood+suite via the already-set env; soak OFF-arm proved that config on-target). Merge is DELICATE: master lives in worktree `KieshStockExchange-soak`, history diverged (Q7 cherry-pick), and a naive `checkout master`+`reset` near-missed the branch tip (recovered from reflog). SAFE PATH = merge in the master worktree w/ build+778-tests gate → box `--env-file .env.production` → set `TakerBias=0.015` → verify on-target. AWAITING Kiesh: proceed-with-worktree-merge vs he drives it. Also pending: rotate prod admin pw → `Petertje60!` + env-var the 6 prod_*.sh.
