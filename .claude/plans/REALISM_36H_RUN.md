...
- **★ DRIFT tilting negative — 1-cycle CONFIRM then ARM log-sym:** @05:51 6h drift **−0.0069** (distribution shifted DOWN — p25/p75 −0.0117/−0.0006, p75 now NEGATIVE vs +0.0055 last cycle). Multi-horizon: 3h/6h/12h/24h/48h **−0.0015/−0.0069/−0.0048/−0.0048/−0.0027 — ALL NEGATIVE**; 24h FLIPPED neg (was +0.0023 @05:09). BUT 24h +0.0023→−0.0048 in 40min = too fast for a structural bias (log-sym targets a SLOW/STABLE down-bias) ⇒ likely a recent transient rotating through the window. ★ DECISIVE RULE: if @06:31 24h STILL <0 ⇒ ARM log-sym A/B immediately (OFF vs GeometricGap+Fundamental+ValueAnchor GeometricBand ON; drift/ret_acf/CK gates). If 24h ≥0 ⇒ transient, HOLD. ret_acf VWAP **−0.081** (on target). Moves VERY calm (45m p95 0.35%/max 1.84%, zero >5%/>3%; 3h zero >3%). CK=0.
