...
- **★ DRIFT oscillating ~0 (stable, on-target):** @13:27 prod 6h drift **−0.0021** (minor neg, p25/p75 −0.0103/+0.0005; recent +0.0008→−0.0009→−0.0021 tiny wander, last 24h was +). ret_acf VWAP **−0.150** (in-band/noisy, not toward −0.4). Moves VERY calm (45m p95 0.27%/max 1.72%, zero >5%/>3%; 3h zero >3%). CK=0. ⇒ HOLD. ✅ prod admin pw ROTATED → `Petertje60!` (verified new=200/old=401; `prod_collect_split_measure.sh` env-var'd to KSE_ADMIN_PASSWORD). DEPLOY (whole-branch merge) still AWAITING Kiesh merge-approach.
