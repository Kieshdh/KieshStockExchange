...
**BUILD ORDER (autonomous):** ✅candle step3(serve-stale) / step2 deferred → ✅F2 PARKED (fork→Kiesh) → ✅F1+F5 combined confirm PASSED (deploy HELD→Kiesh eyeball) → ✅candle seam step7 (`6b873f7`, continuous-open behind default-off `Candles:ContinuousOpen`, 761 tests; HOLD-for-eyeball flip like F3) → **NEXT = client candle cache steps 4-6** (CandleCache read-path class + cache-first gap-only fetch + live-merge all resolutions + reconnect refetch; client-only MAUI, no prod/soak) → F4 → **F3 LAST** (charts-in-chat, hold). Await Kiesh: F2 weak-vs-visible fork; F1+F5 prod deploy+ON-flip (F5 breathe eyeball); candle ContinuousOpen flip (chart eyeball). Attribution rule: never co-enable F1/F2/F5 during a solo soak; F1×F2 compound-cap `VolumeMult×H≤SIZE cap`; F1+F5 combined ret_acf must stay OFF±0.05.
