...
- **★ DRIFT oscillating ~0 (stable, on-target):** @09:19 6h drift **+0.0020** (positive; p25/p75 −0.0074/**+0.0138**). ret_acf VWAP **−0.068** (noisy, neg/in-band). Moves VERY calm (45m p95 0.41%/max 1.45%, zero >5%/>3%; 3h zero >3%, p99 0.85%). CK=0. ⇒ HOLD. (Recent 6h trend −0.0004→+0.0002→−0.0004→+0.0020 = tight wander around 0, no down-bias; log-sym stays default-off. Re-trigger only on STABLE 24h<0 across 2 cycles.)
