...
- **★ DRIFT oscillating ~0 (stable, on-target):** @07:58 6h drift **+0.0002** (flat/positive; p25/p75 −0.0060/**+0.0086**). ret_acf VWAP **−0.063** (noisy, neg/in-band). Moves VERY calm (45m p95 0.38%/max 2.28%, zero >5%/>3%; 3h zero >3%, p99 0.83%). CK=0. ⇒ HOLD. (Recent trend 24h +0.0056 / 6h −0.0023 → +0.0002 = wander around 0, no down-bias; log-sym stays default-off. Re-trigger only on STABLE 24h<0 across 2 cycles.)
