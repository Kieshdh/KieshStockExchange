...
- **★ DRIFT oscillating ~0 (stable, on-target):** @09:59 6h drift **0.0000** (flat, symmetric p25/p75 −0.0073/+0.0090). ret_acf VWAP **−0.055** (noisy, neg/in-band). Moves VERY calm (45m p95 0.26%/max 1.26%, zero >5%/>3%; 3h zero >3%, p99 0.82%). CK=0. ⇒ HOLD. (Recent 6h trend +0.0002→−0.0004→+0.0020→0.0000 = tight wander around 0, no down-bias; log-sym stays default-off. Re-trigger only on STABLE 24h<0 across 2 cycles.)
