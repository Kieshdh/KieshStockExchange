...
**BUILD ORDER (autonomous):** ✅candle step3(serve-stale) / step2 deferred → ✅F2 PARKED (fork→Kiesh) → ✅F1+F5 combined confirm PASSED (deploy HELD→Kiesh eyeball) → ✅candle seam step7 (`6b873f7`, continuous-open behind default-off `Candles:ContinuousOpen`, 761 tests; HOLD-for-eyeball flip like F3) → 🔨candle cache 4-5 LOGIC built+tested (`b269d19` CandleCache pure class in Shared: TryServe/MergeFetched/MergeClosed/InvalidateTail, 13 tests, 774 total; NOT yet wired) → **NEXT = WIRE CandleCache into SignalRCandleService** (cache-first GetHistoricalCandlesAsync w/ bucket-aligned keys + formingBucketOpen; MergeClosed in OnHubCandleClosed; consider default-off `Candles:ClientCache`; verify via client launch) + step6 reconnect hook → F4 → **F3 LAST** (charts-in-chat, hold). Await Kiesh: F2 weak-vs-visible fork; F1+F5 prod deploy+ON-flip (F5 breathe eyeball); candle ContinuousOpen flip (chart eyeball). Attribution rule: never co-enable F1/F2/F5 during a solo soak; F1×F2 compound-cap `VolumeMult×H≤SIZE cap`; F1+F5 combined ret_acf must stay OFF±0.05.
