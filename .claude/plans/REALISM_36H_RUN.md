...
| **F2 VolumeRotation** | `VOLUME_ROTATION_PLAN.md` (STATUS hdr) | 🔨 BUILT default-off `07eca8a` (ORGANIC multi-timescale + daily per Kiesh steer "not so static/dynamic time+daily"; SIZE-only, median-1, 759 tests, CK=0); **A/B SOAK RUNNING** (~01:29 local, 45m; F2off:5080 vs F2on:5083, F1-ON baseline, compressed 8/30min timescales) | analyze soak → council → prod |
