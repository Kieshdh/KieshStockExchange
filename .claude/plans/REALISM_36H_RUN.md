...
- **★ DRIFT oscillating ~0 — negative tilt was TRANSIENT (log-sym NOT armed, correct):** @06:34 24h drift **REVERTED to +0.0056 POSITIVE** (from −0.0048 @05:51) ⇒ per decisive rule 24h≥0 = transient confirmed, HOLD. Full oscillation +0.0023→−0.0048→+0.0056 over ~80min = wander around 0, NO structural down-bias (the 1-confirm-cycle discipline correctly avoided an A/B soak on noise). Multi-horizon @06:34: 3h/6h/12h/24h/48h +0.0019/−0.0002/+0.0013/+0.0056/−0.0009 (all ~0). ret_acf VWAP **−0.044** (noisy, neg/in-band). Moves VERY calm (3h p99 0.80%, zero >3%). CK=0. ⇒ HOLD; drift-watch reset (re-trigger only on a STABLE 24h<0 across 2 cycles, not a 1-cycle flip).
