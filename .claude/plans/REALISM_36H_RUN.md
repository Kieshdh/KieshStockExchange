...
- **★ CLEAN DRIFT + ret_acf — §1 ON-TARGET, drift oscillating ~0 (3-consec tripwire CLEARED by multi-horizon):** @05:09 6h drift **−0.0008** hit 3 consec-neg (−0.0001→−0.0044→−0.0008) ⇒ INVESTIGATED not blind-armed: multi-horizon drift 3h/6h/12h **−0.0017/−0.0010/−0.0026** (tiny noise) but **24h +0.0023 POSITIVE** (on §1 weekly-positive target) ⇒ oscillation around 0, NO down-bias (the 6h windows overlap ~87% so "3 consec" = 1 slightly-neg window re-measured). ret_acf VWAP **−0.107** (dead on §1 −0.1, confirms @04:28 −0.046 was noise). Moves VERY calm (45m p95 0.23%/max 1.45%, zero >5%/>3%; 3h zero >3%, p99 0.69%). CK=0. ⇒ HOLD; do NOT arm log-sym (over-tuning on noise, market on-target); re-trigger only on 24h<0 or larger-magnitude negatives.
