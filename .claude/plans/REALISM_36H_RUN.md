...
- **★ DRIFT oscillating ~0 (stable, on-target):** @08:38 6h drift **−0.0004** (flat ~0; p25/p75 −0.0033/**+0.0122** positive skew). ret_acf VWAP **−0.058** (noisy, neg/in-band). Moves VERY calm (45m p95 0.42%/max 2.41%, zero >5%/>3%; 3h zero >3%, p99 0.87%). CK=0. ⇒ HOLD. (Recent 6h trend −0.0023→+0.0002→−0.0004 = tight wander around 0, no down-bias; log-sym stays default-off. Re-trigger only on STABLE 24h<0 across 2 cycles.)
