...
Compact reference for the market **ENGINE**: the path an order takes from a click or a bot decision to a durable, conserved fill. Companion to `docs/explainers/BOT_MECHANICS.md` — that file covers *who* places orders and *why*; this one covers *what happens to an order once it's placed*. **Consult + UPDATE this file whenever an engine mechanism changes** (same commit).
...
**Companion docs:** `docs/explainers/BOT_MECHANICS.md` (who places orders + the market-realism scorecard), `docs/PERF_SCALING_PLAN.md` (the commit-bound perf work referenced in §3.6), `docs/REALISM_OVERHAUL_PLAN.md` (the taker-flow realism levers).

**Who hosts/drives the engine.** This doc covers the engine as a *library of methods* — something calls `OrderEntryService`/`OrderExecutionService` and the rest follows. *What* calls it, and how the process that owns it is composed and deployed, is **`docs/explainers/SERVER_HOST_AND_OPS.md`**: the `IHostedService` actors that keep the engine live — `BotLoopHostedService` (starts/stops the ~20k-bot `AiTradeService` loop, the primary batch client of §3.7), `StopTriggerWatcher` (the quote-driven armed-stop promoter of §3.8, an always-on `BackgroundService`), and the broadcasters (`MarketHubBroadcaster`/`OrderBookBroadcaster`/`TelemetryBroadcaster`) that push engine events out over SignalR — plus the `Program.cs` DI graph (including the `Lazy<IStopWatcher>` cycle-break and the thrice-registered `StopTriggerWatcher` singleton), config layering, Docker/compose, and the deploy/rollback runbook. Read it when the question is "who ticks this / how does it run in prod," not "what happens to one order."
