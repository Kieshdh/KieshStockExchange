...
Compact reference for **every network boundary a client (or ops tool) can hit** on `KieshStockExchange.Server`: the REST controllers, their auth requirements, the trading/auth request+response shapes, and the one SignalR hub. Companion to `docs/ENGINE_MECHANICS.md` (what happens to an order *after* it's placed), `docs/BOT_MECHANICS.md` (who places orders), and `docs/CLIENT_STRUCTURE.md` (the MAUI client that consumes this surface — its REST-vs-SignalR map is mirrored here from the **server** side, §4).
