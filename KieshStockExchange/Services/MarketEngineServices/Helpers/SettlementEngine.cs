...
public sealed class SettlementEngine : ISettlementEngine
{
        ...
        CancellationToken ct = default)
    {
        ...
        AssertConservation(fundMap, fundSnapshots, posMap, posSnapshots, accepted);
    ...
    /// <summary>
    /// Money- and share-conservation probe. Sums (post − pre) on every Fund/Position
    /// the call mutated; the apply-pass is symmetric per fill so both sums must be zero
    /// (modulo currency rounding for funds, exact zero for share counts). A non-zero
    /// result means a mutation fired on one side but not the other — the kind of
    /// asymmetry that would silently mint cash or shares. Logged at Error level with
    /// the first trade as a clue; never throws, so a probe-only build can keep running.
    /// </summary>
